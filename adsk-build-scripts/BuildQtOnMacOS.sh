...
if [ $QT_BUILD_RELEASE_ENABLED -eq 1 ]; then
  ...
  # Remove GPL-licensed tool translations (designer, assistant, linguist, qt_help)
  # These are generated by qttranslations regardless of -no-feature-designer
  rm -f ${CONFIG_PREFIX_RELEASE}/translations/assistant_*.qm
  rm -f ${CONFIG_PREFIX_RELEASE}/translations/designer_*.qm
  rm -f ${CONFIG_PREFIX_RELEASE}/translations/linguist_*.qm
  rm -f ${CONFIG_PREFIX_RELEASE}/translations/qt_help_*.qm
fi
...
if [ $QT_BUILD_DEBUG_ENABLED -eq 1 ]; then
  ...
  # Remove GPL-licensed tool translations (designer, assistant, linguist, qt_help)
  # These are generated by qttranslations regardless of -no-feature-designer
  rm -f ${CONFIG_PREFIX_DEBUG}/translations/assistant_*.qm
  rm -f ${CONFIG_PREFIX_DEBUG}/translations/designer_*.qm
  rm -f ${CONFIG_PREFIX_DEBUG}/translations/linguist_*.qm
  rm -f ${CONFIG_PREFIX_DEBUG}/translations/qt_help_*.qm
fi
#################################################
