...
set +e
html5lib_installed=$(python -m pip list installed 2>/dev/null | grep html5lib | wc -l)
if [ $html5lib_installed -eq 0 ]; then
    echo "python html5lib not installed. Installing it."
    python -m pip install --no-input html5lib
    if [[ $? -ne 0 ]]; then
        echo "Install of python html5lib failed. aborting."
        exit 1
    fi
fi
python -m pip list installed 2>/dev/null | sed -e '/html5lib/!d;s/[[:space:]][[:space:]]*/ /g'
set -e
