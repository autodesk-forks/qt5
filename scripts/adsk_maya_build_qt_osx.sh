...
$SRCDIR/configure -opensource -confirm-license -verbose -prefix $INSTALLDIR -debug-and-release -force-debug-info -separate-debug-info -nomake tests -nomake examples -plugin-sql-sqlite -silent -no-strip -skip qtnetworkauth -skip qtpurchasing -no-warnings-are-errors -framework
if [ $? -eq 0 ]; then
